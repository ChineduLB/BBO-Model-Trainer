...
def train_side_model(df: pd.DataFrame, side: str):
    ...
    # Base positive rate in test (what random guessing would get on average)
    base_rate_test = y_test.mean()
    print(f"[{side}] Test base positive rate (TP-first frequency): {base_rate_test:.4f}")
    ...
    coverage = y_test_pred.mean()

    print(
        f"[{side}] A={A:.6f}, B={B:.6f}, thr={thr:.6f}, "
        f"val_prec={val_prec:.4f}, "
        f"test_prec_recent={test_prec:.4f}, "
        f"test_rec_recent={test_rec:.4f}, "
        f"coverage_recent={coverage:.4f}"
    )

    # -------------------------------------------------------------------------
    # Extra diagnostics: precision / recall / coverage at a few thresholds
    # -------------------------------------------------------------------------
    print(f"\n[{side}] Precision/Recall/Coverage on recent test for various thresholds:")
    # Use a sorted unique list of candidate thresholds (including the chosen thr)
    candidate_thresholds = sorted(set([0.5, 0.6, 0.7, thr, 0.8]))
    for thr_c in candidate_thresholds:
        y_pred_c = (p_test_cal >= thr_c).astype(int)
        prec_c = precision_score(y_test, y_pred_c, zero_division=0)
        rec_c = recall_score(y_test, y_pred_c, zero_division=0)
        cov_c = y_pred_c.mean()
        print(
            f"  thr={thr_c:.3f}: "
            f"prec={prec_c:.3f}, rec={rec_c:.3f}, coverage={cov_c:.3f}"
        )
...
def main():
    ...
    # Train Long / Short with diagnostics
